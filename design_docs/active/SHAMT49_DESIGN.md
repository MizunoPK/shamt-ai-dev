...
| ADO CI / ADO MCP | Only when `pr_provider.conf` contains `ado` |
...
- [ ] Read config sources: `epic_tag.conf` (default `SHAMT-N` if absent) and the three pre-existing conf files written by `init.sh` — `ai_service.conf`, `repo_type.conf`, and `pr_provider.conf` (default `github` if absent) — plus `.claude/settings.json`
...
- [ ] Write `.shamt/.gitignore` containing `CHEATSHEET.md` so the generated file is gitignored in projects that track `.shamt/` (nested `.gitignore` files are respected by git; this is a no-op in projects where `.shamt/` is already gitignored at the repo level)
...
- [ ] Add Phase 5 to `regen-codex-shims.sh` by copying the Phase 2 inline Python3 script verbatim, then wrapping the `json.load(settings_json)` call in a try/except that catches `FileNotFoundError` and `json.JSONDecodeError` (same pattern as Phase 2's existing fallback in step 6) and defaults both `hooks_enabled` and `mcp_enabled` to `False` — no other changes needed
...
1. **Codex dual-host:** For `ai_service=claude_codex` projects, the cheat sheet should contain both Claude Code and Codex sections. The filtering logic handles this (include a section when the service string *contains* `claude` or *contains* `codex`). Confirm this is the right behavior before implementing.

2. **MCP detection method:** The design reads `mcpServers.shamt` from `settings.json` to determine if MCP is enabled. On Codex-only projects there is no `settings.json`. Confirm fallback: treat MCP as disabled if `settings.json` is absent.
