...
6. **Validate implementation:** Run implementation validation loop (see design doc Proposal 10 pattern). Use `shamt.validation_round()` with `exit_threshold=1`. See [`composites/validation_loop_composite.md`](../composites/validation_loop_composite.md) for optional `/loop` self-pacing when context exhaustion is a risk.
7. **Guide audit:** Run the full guide audit (3 consecutive clean rounds required; ≤1 LOW per round is clean). See [`composites/master_review_pipeline_composite.md`](../composites/master_review_pipeline_composite.md) for the assembled guide audit flow.
