...
9. [Prompt 8: Sync Timeout (Secondary at S3/S4→S5)](#prompt-8-sync-timeout-secondary-at-s3s4s5)
...
✅ I will run S3 SOLO after all features complete S2, then S4 SOLO for interface contracts
...
✅ I will NOT run S3 myself - Primary runs S3 solo (S4 = Interface Contract Definition)
...
## Prompt 8: Sync Timeout (Secondary at S3/S4→S5)

**When to use:** Waiting for Primary to complete S3 and S4, exceeded time threshold
...
**Situation:** Primary started S3/S4 {elapsed_hours} hours ago. Have not received S4 completion signal.
...
2. New Agent Runs S3 (if Primary stale) - S4 runs separately

**Note:** I cannot proceed to S5 without S3 and S4 complete. Both require epic-level view that only Primary has.
...
2. Coordination Heartbeat (every 15 minutes during S2)
3. Sync Timeout at S3/S4→S5 (if Primary delayed)
