...
- Created: `.shamt/guides/audit/dimensions/d21_agent_comprehension_risk.md`
- Reason: New standalone audit dimension (D21) for per-guide agent comprehension risk. Consolidates and formalizes the three failure patterns identified in the S3 scope investigation: (1) scope stated only in prerequisites rather than prominently near H1, (2) migration/authoring notes appearing in the agent instruction path, (3) structurally similar sibling guides without explicit scope differentiation callouts. D18 additions above cover these at the cross-stage transition level; D21 covers them at the individual-guide level. Updated audit_overview.md to register D21 in the dimension list, sub-round N.4, dimension count references, and exit criteria.
